...
/*   open files for output if wanted                                          */
    ...
    if ( name[j] == '\0' ) {
        ...
        if ( k - j < 8 ) strncat_s(name1,xxx,8-k+j);
    }
    if ( ! silent ) 
    {
        strcpy(fil,name1);
		meu  = fopen(strcat(fil,".mes"),"w");
	//	fopen_s(&meu, strcat(fil,".mes"), "w");

        strcpy(fil,name1);
        prou = fopen(strcat(fil,".pro"),"w");
	//	fopen_s(&prou, strcat(fil, ".pro"),"w");
    }
...
/******************************************************************************/
    ...
    for ( i = 1 ; i <= nres ; i++ )
    {
       ...
       if (! (low[i] == up[i]) )
       { 
          del0 = o8min ( del0 , (up[i]-low[i])/(four*term) );
       }
    }
    delmin = tm6;
 ...
 /*   rescale the gradients of the linear constraints */
      ...
      gresn[i] = o8max ( one , sqrt(gresn[i]) ) ;
      cgres[i] = 1 ;
      val[i]   = TRUE ;
    }
...
void o8st(void) {
    ...
    rho1   = tm10;
    del01  = del0/tp1;
    delmin = o8min(del01,o8max(tm6*del0,smallw));
    if ( ! analyt ) delmin = o8min(del01,o8max(epsdif,delmin));
...
void o8fin(void) {
    ...
    for ( k = 1 ; k <= nres ; k++ ) {
        if ( low[k] != up[k]  ) umin =o8min( o8min( umin,u[2*k-1] ) ,u[2*k]);
    }
    crtot  = i;
    cgrtot = j;
    nsing  = 0;
    ...
    if ( te1 ) {
        for (i = 1 ; i <= itstep ; i++) {
            ih1 = (INTEGER)accinf[i][1];
            ih2 = (INTEGER)accinf[i][9];
            ih3 = (INTEGER)accinf[i][10];
            ih5 = (INTEGER)accinf[i][18];
            ih6 = (INTEGER)accinf[i][19];
            ih7 = (INTEGER)accinf[i][22];
            ih8 = (INTEGER)accinf[i][26];
            ih9 = (INTEGER)accinf[i][27];
    ...
    /* 24: o8dscal  scaling factor for d                                          */
...
void o8info(INTEGER icase) {
    ...
    case 8:
        ...
        if ( intakt ) {
            ...
            phih,dirder,psi,y);
            printf(  "     fx= %11.4e  o8dscal= %11.4e    scf= %11.4e   upsi= %11.4e\n", 
            fx,o8dscal,scf,upsi);
        }
        fprintf(prou,"\n\n start unimin\n\n");
        fprintf(prou,"    phi= %11.4e   dphi= %11.4e    psi= %11.4e tau0/2= %11.4e\n",
        phih,dirder,psi,y);
        fprintf(prou,"     fx= %11.4e  o8dscal= %11.4e    scf= %11.4e   upsi= %11.4e\n",
        fx,o8dscal,scf,upsi);
...
/*  gives two inequality constraints                                       */ 
            ...
            s1 = s1-o8min(zero,resst[2*i-1])*w1[2*i-1];
            s2 = s2-o8min(zero,res[2*i-1]  )*w1[2*i-1];
            s1 = s1-o8min(zero,resst[2*i])*w1[2*i];
            s2 = s2-o8min(zero,res[2*i]  )*w1[2*i];

        }
    }
    diff0 = (fxst-fx)*scf+(s1-s2);
    if ( wlow && diff0 >= eta*clow && itstep-lastdw > o8max(5,o8min(20,n/10)) ) 
    {
    ...
    else 
    {
        ...
        for (i = 1 ; i <= nres ; i++) {
            ...
            w[2*i-1] = o8max(w[2*i-1],w1[2*i-1]);
            w[2*i]   = o8max(w[2*i],w1[2*i]);
            if ( low[i] == up[i] ) {
                s1 = s1+w[2*i-1]*fabs(resst[2*i-1]);
                s2 = s2+w[2*i-1]*fabs(res[2*i-1]);
            } else {
                s1 = s1-w[2*i-1]*o8min(zero,resst[2*i-1]);
                s2 = s2-w[2*i-1]*o8min(zero,res[2*i-1]);
                s1 = s1-w[2*i]*o8min(zero,resst[2*i]);
                s2 = s2-w[2*i]*o8min(zero,res[2*i]);
            }
        }
        psist = s1;
        psi   = s2;
...
void o8opti(void) 
{
    ...
    static DOUBLE   compl_,del1;
    static INTEGER  iumin,rank0,nr0,csdifx,clwold;
    static INTEGER  nrbas;
    ...
    eps = o8max(epsmac*tp3,o8min(tm3,eps));
...
/* the unscaled penalty function                                      */
    {
       ...
       {
          term =  - o8min(zero,res[2*i-1]);
          term1=  - o8min(zero,res[2*i]);
          if ( res[2*i-1] <= delmin )
          {
            aalist[0]       += 1 ;
            aalist[aalist[0]] = 2*i-1 ;
...
/* be active simultaneously                                                 */
    ...
    for ( i = 1 ; i <= clist[0] ; i++ )
    {
       ...
       gresn[clist[i]+nlin] = o8max ( one , sqrt(o8sc4(1,n,clist[i]+nlin,gres)) );
    }
        ...
        || accinf[itstep-1][10] == 1. ) ) 
    {
        ...
        for (i = 1 ; i <= nlin+nonlin ; i++) 
        {
             term = o8max(term,gresn[i]);
        }
        scfh = term/o8max(one/scfmax,gfn);
    ...
    {
        etaini = TRUE;
        eta    = (accinf[itstep-1][8]/o8max(one,gfn)+sum
                 +o8min(one,slackn)+o8min(one,fabs(uminsc)))/o8min(30*n,iterma);
        level  = eta;
    }
    if ( itstep > 1 ) 
    { 
    ...
    if ( itstep > 1 && term > zero && scf != zero ) 
       delx = o8max(delx,exp(p7*p7*log(term)));
    if ( scf == zero ) delx = o8min(del0*tm4,o8max(delx,upsi*tm2));
    delsig = delmin;
    ...
    for (i = 1 ; i <= violis[0] ; i++) 
    {
        ...
        delsig = o8max(delsig,res[j]/denom*(one+tm1));
    }
    del = o8min(del0,o8max(o8min(delsig,five*delx),delx));


    if ( violis[0] == 0 ) del = o8min(del,del01);
    ...
    if ( phase == 2 && violis[0] == 0 ) 
    {
        for (i = 1 ; i <= nres ; i++) 
        {
            if ( !( low[i]==up[i]) ) 
            {
              ...
              if ( i > n ) term /= gresn[i-n];
              else term /= o8max(one,xsc[i]);
              
              del = o8min(del01,o8max(del,term));
            }
        }
    }
    ...
    for (i = 1 ; i <= o8opti_delist[0] ; i++) 
    {
        ...
        if ( term1 >= del*tm2 ) term = o8min(term,term1);
    }
    del = term;
    ...
    if ( itstep > 1 && ! ident && scf != zero ) 
    {
        ...
        for (i = 1 ; i <= nres ; i++) 
        {
            ...
            { 
               ...
               {
                 term /= o8max(one,xsc[i]);
                 term1 /= o8max(one,xsc[i]);
               }
               term = o8max ( zero , term-delmin );
               term1 = o8max ( zero , term1 - delmin ) ;
               term *= o8max(zero,u[2*i-1]-smallw);
               term1 *= o8max(zero,u[2*i]-smallw);
               if ( i > n )
               {
                 term /= gresn[i-n];
                 term1 /= gresn[i-n];
        ...
        if ( compl_ > zero ) 
        {
           for (i = 1 ; i <= nres ; i++) 
           {
              ...
              { 
                ...
                {
                    del = o8max(delmin*tp2,
                              o8min(del,term*(one-tm2)));
                }
                if ( u[2*i] > smallw && term1 > tp2*delmin )
                {
                    del = o8max(delmin*tp2,
                              o8min(del,term1*(one-tm2)));
                }
              }
           }
        }
    ...
    if ( itstep > 1 && accinf[itstep-1][30] < 0. ) del =o8min(tp1*delold,del0);
    ...
    for (i = 1 ; i <= nres ; i++) 
    {
       if ( !(low[i] == up[i] ) )
       {
        ...
        if ( o8min(term,term1) <= del && (o8bind[2*i-1]+o8bind[2*i] ==0 ) )
        {
          j=2*i-1;
          if ( term1 < term ) j=2*i;
...
/* inactive  
    ...
    del1 = del;
    if ( b2n > zero ) del1 = o8max(del,tm1*o8min(del0,
    exp(p7*log( fabs(b2n)/(gfn+one) + o8max(zero,-uminsc) + sum)) ));
    ...
    for (i = 1 ; i <= o8opti_delist[0] ; i++) 
    {
        ...
        {
           term1 /= o8max(one,xsc[(j+1)/2]);
        }
        if ( term1 >= del1*tm2 ) del1 = o8max(delmin,o8min(del1,term1));
    }
    ...
    for (i = 1 ; i <= nres ; i++) 
    {
        ...
        slackn += 
           o8max(zero,term/denom-delmin)*o8max(zero,u[2*i-1]-smallw)/denom
          +o8max(zero,term1/denom-delmin)*o8max(zero,u[2*i]-smallw)/denom;
    }
    ...
    for (i = 1 ; i <= nres ; i++) 
    {
      if ( low[i] != up[i] )
      {
        ...
        if ( o8min(term,term1) <= del1 && (o8bind[2*i-1]+o8bind[2*i] == 0 ))
        {
    ...
    if ( phase >= 0 && b2n != -one ) 
    {
        if ( fabs(uminsc) >= o8max(smallw,fabs(b2n)/(gfn+one)*c1d)) 
        {
            for (i = 1 ; i <= nr ; i++) 
            {
                k = aalist[colno[i]];
    ...
    if ( nr > 1 ) 
    {
        ...
        for (i = 1 ; i <= nr ; i++) 
        {
            term  = o8max(term, fabs(diag[i]));
            term1 = o8min(term1,fabs(diag[i]));
        }
    ...
    for (i = 1 ; i <= nres ; i++) 
    {
        term = res[2*i-1];
        term1 = res[2*i]; 
        denom = (i > n ) ? gresn[i-n] : o8max(one,xsc[i]);
        slackn +=
           o8max(zero,term/denom-delmin)*o8max(zero,u[2*i-1]-smallw)/denom
          +o8max(zero,term1/denom-delmin)*o8max(zero,u[2*i]-smallw)/denom;
    }
    ...
    if ( phase >= 0 && upsi > tau0*p5 )
         scf0 = o8max(one/scfmax,(two*(tau0-upsi)/tau0)*upsi*tm1/o8max(one,gfn) )/scf;
    ...
    if ( clow > clwold ) 
    {
        ...
        tauqp = o8max(one,o8min(tauqp,term));
    }
    /* compute parameter phase and stopping criterion */

    if ( uminsc < -smallw )     phase = o8min(1,phase);
    if ( ! eqres )              phase = o8min(0,phase);
    if ( eqres && upsi < tau0 ) phase = o8max(1,phase);
  ...
  /*  active at the old point also at the new point (necessary for the update)*/
    ...
    if ( term != zero && phase >= 0 ) matsc = o8max(one/scfmax,o8min(scfmax,term/2));
  ...
  /*  active at the new point but not at the old point                        */
    ...
    if ( phase >= 0 && upsi > tau0*p5 ) 
    {
        scfh = o8max(one/scfmax,
               o8min(scfmax,(two*(tau0-upsi)/tau0)*upsi*tau/o8max(one,gfn) ) );
        if ( (fxst-fx)*scfh+scfh/scf*(psist-psi) >= 
             scfh/scf*eta*clow && lastch <= itstep-4
             && (scfh < tm1*scf || scfh > tp1*scf) )
        {
    ...
    if ( dnorm <= epsx*(o8min(xnorm,one)+epsx) && qpterm < 0 ) 
    ...
    if ( qpterm >= 0 && dnorm <= tm2*epsx*(epsx+o8min(one,xnorm))) 
    {
        if ( upsi <= nres*delmin ) 
        {
            optite = six;
       ...
       /*  bound constraints                                    */
          ...
          else
          {
            ...
            /*  active lower bound */
            {
              ...
              {
                dirder -= o8min(zero,term);
              }
              else
              { 
                if ( res[2*i-1] < -tp3*epsmac )
                {
                  term = o8min ( term , -res[2*i-1]*w[2*i-1] );
...
void o8cutd(void) 
{
    ...
    for (i = 1 ; i <= n ; i++) 
    {
        ...
        {
            d[i]  = zero;
            dd[i] = o8max(zero,dd[i]);
        }
        if ( lup[i] && x[i]+sigsm*d[i] >= og[i] ) 
        {
            d[i]  = zero;
            dd[i] = o8min(zero,dd[i]);
        }
    }
    dnorm = o8vecn(1,n,d);
        ...
        for (i = 1 ; i <= n ; i++) {
            if ( llow[i] && d[i] < zero ) {
                if ( -d[i]*sigla >= x[i]-ug[i] ) {
                    stmaxl = o8max(stmaxl,(x[i]-ug[i])/(-d[i]));
                } else {
                    stmaxl = sigla;
                }
            }
            if ( lup[i] && d[i] > zero ) {
                if ( d[i]*sigla >= og[i]-x[i] ) {
                    stmaxl = o8max(stmaxl,(og[i]-x[i])/d[i]);
                } else {
                    stmaxl = sigla;
                }
            }
...
void o8eval(DOUBLE sigact,DOUBLE *sigres,LOGICAL *reject,LOGICAL *error) 
{
    ...
    for (i = 1 ; i <= n ; i ++) 
    {
        ...
        x1[i] = o8max(x1[i],ug[i]);
        x1[i] = o8min(x1[i],og[i]);
    }
    *reject = FALSE;
    *error  = FALSE;
    *sigres = sig;
...
/********** compute res1 = restrictions at x1     *******************/
    ...
    for ( i = 1 ; i <= nres ; i++ )
    {
      if ( low[i] == up[i] ) 
      {
         ...
         denom = ( i<=n ) ? o8max(one,xsc[i]) : gresn[i-n];
         ...
         if ( res1[2*i-1]*res[2*i-1] < zero && sig <= one && 
            ...
            *sigres = o8min(*sigres,sig*res[2*i-1]/(res[2*i-1]-res1[2*i-1]));
      }      
      else
      {
         term = -o8min(zero,res1[2*i-1]);
         ...
         if ( res1[2*i]*res[2*i] < zero && sig <= one && ( o8bind[2*i] == 0 ||
            ...
            *sigres =o8min(*sigres,sig*res[2*i]/(res[2*i]-res1[2*i]));
...
void o8unim(DOUBLE sig1th) {
    ...
    if ( lastch >= itstep-3 || phase != 2 || singul ) {
        ...
        maxphi = phi;
        for (j = 1 ; j <= 3 ; j++) {
            maxphi = o8max(scf*accinf[itstep-j][2]+accinf[itstep-j][4],maxphi);
        }
        diff = maxphi-phi1;
    }
    desc   = diff >= o8min(-sig*delta*dirder,level);
    ...
    if ( sigres < sig ) {
        sig = o8min(p5*sig,o8max(o8unim_step[l]*sig,sigres));
    } else {
        term = (diff-dirder*sig)*two;
        if ( term > epsmac*(scf*fabs(fx)+psi) ) {
            sig = o8min(p5*sig,o8max(o8unim_step[l]*sig,-dirder*pow(sig,2)/term));
        } else {
            sig = o8unim_step[l]*sig;
        }
    }
    ...
    if ( sig*o8max(one,dnorm) >= sigsm ) goto L100;
    stptrm = -one;
    sig    = zero;
...
/* above the diagonal of qr. o8cscal is the column scaling of the                 */
...
void o8dec(INTEGER nlow,INTEGER nrl) {
    ...
    for ( i = nlow ; i <= nrl ; i++) {
        ...
        if ( sum == zero ) {
            ...
            term     = one/sqrt(o8max(sum,pow(rho,2)));
            o8cscal[i] = term;
            if ( nlow > 1 ) {
...
void o8sol(INTEGER nlow,INTEGER nup,DOUBLE b[],DOUBLE x[]) {
    ...
    for (i = nlow ; i <= nup ; i++) {
        x[i] = o8sol_xl[i]*o8cscal[colno[i]];
    }
...
void o8qpdu(void) 
{
   ...
   {
       tiny = 2*nr*o8max(epsdif,epsmac*tp3);
   }
   ...
   /* restart point in case of increase of tauqp */
    ...
    {
        c1 = o8max(c1,fabs(a[i][i]));
        c2 = o8max(c2,fabs(xj[i+incr][i+incr]));
    }
    my = zero;
    for (i = 1 ; i <= n ; i++) 
    {
...
/* end computation of data of mi-th QP constraint data */
    ...
    if ( fabs(psid) <= tiny*(c1*c2+condr) || ip == 0) {
        ...
        if ( infe1 > (one-delta1/tauqp)*upsi &&
            ( o8vecn(1,n,d) <=o8min(infe1,pow(infe1,2))*tp1
            || upsi > tau0*p5 ) ) 
        {
        ...
        for (i = 1 ; i <= nres ; i++) 
        {
            ...
            else 
            {
               if ( o8bind[2*i-1] == 0 ) 
               {
                  w1[2*i-1] = o8max(w[2*i-1]*p8,tau);
               } 
               else 
               {
                  if ( res[2*i-1] >= zero && slack[2*i-1] <= tiny )
                    w1[2*i-1] = 
                    o8max(ny*fabs(u[2*i-1])+tau,(fabs(u[2*i-1])+w1[2*i-1])*p5);
                  if ( res[2*i-1] >= zero && slack[2*i-1] > tiny )
                    w1[2*i-1] = fabs(u[2*i-1]);
               ...
               if ( o8bind[2*i] == 0 ) 
               {
                  w1[2*i] = o8max(w[2*i]*p8,tau);
               } 
               else 
               {
                  if ( res[2*i] >= zero && slack[2*i] <= tiny )
                    w1[2*i] = 
                    o8max(ny*fabs(u[2*i])+tau,(fabs(u[2*i])+w1[2*i])*p5);
                  if ( res[2*i] >= zero && slack[2*i] > tiny )
                    w1[2*i] = fabs(u[2*i]);
        ...
        if ( wlow ) 
        {
            ...
            for (i = 1 ; i <= nres ; i++) 
            {
                ...
                {
                    s1 -=o8min(zero,resst[2*i-1])*w1[2*i-1];
                    s2 -=o8min(zero,res[2*i-1]  )*w1[2*i-1];
                    s1 -=o8min(zero,resst[2*i])*w1[2*i];
                    s2 -=o8min(zero,res[2*i]  )*w1[2*i];

                }
            }
            diff0 = (fxst-fx)*scf+(s1-s2);
            if ( diff0 >= eta*clow && itstep-lastdw >= o8max(5,o8min(n/10,20)) ) {
        ...
        for (i = 1 ; i <= nres ; i++  ) 
        {
            ...
            if ( low[i] == up[i] ) 
            {
                ...
                if ( slack[2*i-1] <= fabs(res[2*i-1]) ) 
                {
                  ...
                  {
                      w1[2*i-1] = o8max(w[2*i-1],ny*fabs(u[2*i-1])+tau);
                  }
                }
            } 
            else 
            {
                if ( slack[2*i-1] > -o8min(-tiny,res[2*i-1]) && o8bind[2*i-1] == 1) 
                {
                   w1[2*i-1] = fabs(u[2*i-1]);
                } 
                else if(  o8bind[2*i-1] == 1 && 
                          slack[2*i-1] <= -o8min(-tiny,res[2*i-1])
                ...
                if ( slack[2*i] > -o8min(-tiny,res[2*i]) && o8bind[2*i] == 1 )
                {
                   w1[2*i] = fabs(u[2*i]);
                }
                else if ( o8bind[2*i] == 1 && slack[2*i] <= -o8min(-tiny,res[2*i])
                ...
                {
                    w1[2*i] = o8max(w[2*i],ny*fabs(u[2*i])+tau);
                }
        ...
        for (i = 1 ; i <= nres ; i++) {
            if ( low[i] == up[i] ) 
            {
                ...
                s1 -= w[2*i-1]*o8min(zero,resst[2*i-1]);
                s2 -= w[2*i-1]*o8min(zero,res[2*i-1]);
                s1 -= w[2*i]*o8min(zero,resst[2*i]);
                s2 -= w[2*i]*o8min(zero,res[2*i]);
   ...
   /************************************************************/
    ...
    for (i = 1 ; i <= nres ; i++) 
    {
      ...
      else 
      {
        ...
        else if ( o8bind[2*i-1] == 1 && slack[2*i-1] <= -o8min(-tiny,res[2*i-1])
                  && u[2*i-1] <= w[2*i-1]+tau && w[2*i-1] >= u[2*i-1] ) 
        {
           w1[2*i-1] = w[2*i-1]+two*tau;
        } 
        ...
        su1 -= w1[2*i-1]*o8min(zero,res[2*i-1]);
        su2 -= w1[2*i-1]*o8min(zero,resst[2*i-1]);
        ...
        su1 -= w1[2*i]*o8min(zero,res[2*i]);  
        su2 -= w1[2*i]*o8min(zero,resst[2*i]);
...
void o8dlcd(INTEGER ai[],INTEGER l) {
    ...
    if ( iq >= 1 ) {
        ...
        while ( i < iq ) {
            i     = i+1;
            rnorm = o8max(rnorm,fabs(r[i][i]));
            rlow  = o8min(rlow, fabs(r[i][i]));
        }
    }
    return;
}
...
void esgradf(DOUBLE x[],DOUBLE gradf[]) 
{
    ...
    else 
    {
        ...
        else 
        {
            if ( difftype == 1 ) 
            {
                deldif =o8min(tm1*sqrt(epsfcn),tm2);
                ...
                {
                    xhelp = xtr[j];
                    xincr =o8min(o8min(tm2,deldif*fabs(xhelp)+deldif),taubnd);
                    if ( xhelp >= zero ) 
                    {
                        xtr[j] = xhelp+xincr;
                    }
            ...
            {
                deldif =o8min(tm1*pow(epsfcn,one/three),tm2);
                for (j = 1 ; j <= n ; j++) 
                {
                    xhelp  = xtr[j];
                    xincr  =o8min(o8min(tm2,deldif*fabs(xhelp)+deldif),taubnd);
            ...
            {
                deldif =o8min(tm1*pow(epsfcn,one/seven),tm2);
                for (j = 1 ; j <= n ; j++) 
                {
                   xhelp  = xtr[j];
                   xincr  =o8min(o8min(tm2,deldif*fabs(xhelp)+deldif),taubnd/four);
                   xtr[j] = xhelp-xincr;
...
void global_mem_malloc() {
    ...
    o8cscal      = d1_malloc(n+nlin+nonlin+1, 1);
